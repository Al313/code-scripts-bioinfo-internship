...
# getting th snv_counts in the main data matrix
...
# chromatin_states <- NA
# signature_chromatin_matrix <- data.frame(sampleId = NA, primaryTumorLoc = NA, pathway = NA, sub_pathway = NA, hgnc_symbol = NA, snv_count = NA, chromatin_status = NA)
# signature_chromatin_matrix[mut_sigs] <- NA
# k <- 1
# 
...
# 
#   if (is.na(chromatin_states[1])){
#     chromatin_states <- names(table(vcf$chromatin_status_tads))[c(1,2,5,6,9,10)]
# 
#   }
# 
#   for (j in chromatin_states){
# 
#     signature_chromatin_matrix[k,c(1,2,3,4,5,6)] <- metadata[i,c(2,7,29,30,31,32)]
#     signature_chromatin_matrix[k,7] <- j
# 
#     nn <- names(table(vcf$mut_sign[vcf$chromatin_status_tads == j]))
#     vv <- as.vector(table(vcf$mut_sign[vcf$chromatin_status_tads == j]))
# 
#     signature_chromatin_matrix[k,nn] <- vv
# 
#     k <- k + 1
#   }
# 
...
# 
# 
# saveRDS(signature_chromatin_matrix, file = "./mut_sig_chrom_matrix.rds")




if (dir.exists("/hpc/cuppen/")){
  signature_chromatin_matrix <- readRDS(file = "./mut_sig_chrom_matrix.rds")
} else {
  signature_chromatin_matrix <- readRDS(file = "/home/ali313/Documents/studies/master/umc-project/hpc/cuppen/projects/P0013_WGS_patterns_Diagn/misc/processed/ali-proj/analysis/signature-exploration/sig-and-chrom-status/mut_sig_chrom_matrix.rds")
}
...
for (sample_id in metadata$sampleId){
  ...
  for (mut_sig in mut_sigs){
    ...
    signature_chromatin_matrix[signature_chromatin_matrix$sampleId == sample_id, mut_sig] <- signature_chromatin_matrix[signature_chromatin_matrix$sampleId == sample_id, mut_sig]/sum(signature_chromatin_matrix[signature_chromatin_matrix$sampleId == sample_id, mut_sig])
...
my_df <- data.frame(pathway = NA, chrom_status = NA)
my_df[colnames(signature_chromatin_matrix)[8:61]] <- list(character())
i <- 0

for (chrom_stat in unique(signature_chromatin_matrix$chromatin_status)){
  for (pathway in unique(signature_chromatin_matrix$pathway)[c(1,2,4,5,6)]){

    i <- i + 1
    my_df[i,"pathway"] <- pathway
    my_df[i,"chrom_status"] <- chrom_stat

    for (mut_sig in mut_sigs){
      print(mut_sig)
      my_df[i,mut_sig] <- mean(signature_chromatin_matrix[signature_chromatin_matrix$pathway == pathway & signature_chromatin_matrix$chromatin_status == chrom_stat, mut_sig], na.rm = T)
...
# making a tibble
signature_chromatin_tibble <- gather_(my_df,
                                      key_col = "mut_sigs",
                                      value_col = "value",
                                      gather_cols = colnames(my_df[,3:56]))
...
signature_chromatin_tibble[,1] <- factor(signature_chromatin_tibble[,1], levels = c("WT","BER","NER","POL", "MMR"))
signature_chromatin_tibble[,2] <- factor(signature_chromatin_tibble[,2], levels = c("Active", "Active-2", "Heterochromatin", "Inactive", "Repressed", "TAD_bd"))
signature_chromatin_tibble[,3] <- factor(signature_chromatin_tibble[,3], levels = sig_order)
signature_chromatin_tibble[,4] <- as.numeric(signature_chromatin_tibble[,4])
...
signature_chromatin_tibble1 <- signature_chromatin_tibble[signature_chromatin_tibble$mut_sigs %in% c("SBS1","SBS2","SBS10a","SBS15","SBS44"),]
sample_count1 <- sample_count[sample_count$signature %in% c("SBS1","SBS2","SBS10a","SBS15","SBS44"),]
...
for (i in 1:nrow(data_to_be_removed)) {
  signature_chromatin_tibble$value[signature_chromatin_tibble$mut_sigs == data_to_be_removed$mut_sigs[i] & signature_chromatin_tibble$pathway == data_to_be_removed$pathway[i]] <- NA
}
...
if (dir.exists("/hpc/cuppen/")){
  ggsave(filename = "/home/ali313/Documents/studies/master/umc-project/hpc/cuppen/projects/P0013_WGS_patterns_Diagn/misc/processed/ali-proj/analysis/signature-exploration/sig-and-chrom-status/pathway-mutsig-chromstat-corrected.pdf",
         plot = m, width = 50, height = 110, units = "cm")
} else {
  ggsave(filename = "pathway-mutsig-chromstat-corrected.pdf", plot = m, width = 50, height = 110, units = "cm")
  ggsave(filename = "/home/ali313/Documents/studies/master/umc-project/hpc/cuppen/projects/P0013_WGS_patterns_Diagn/misc/processed/ali-proj/analysis/signature-exploration/sig-and-chrom-status/pathway-mutsig-chromstat-corrected.pdf", plot = m, width = 50, height = 110, units = "cm")
}
