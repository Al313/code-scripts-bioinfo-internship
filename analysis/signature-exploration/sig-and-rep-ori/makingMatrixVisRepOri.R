# ===========| rep-ori and mut-sig visualization |===============
...
# 
# rep_ori <- NA
# signature_rep_ori_matrix <- data.frame(sampleId = NA, primaryTumorLoc = NA, pathway = NA, sub_pathway = NA, hgnc_symbol = NA, snv_count = NA, rep_ori = NA)
# signature_rep_ori_matrix[mut_sigs] <- NA
# k <- 1
# 
...
# 
#   if (length(rep_ori)==1){
#     rep_ori <- names(table(vcf$rep_str_ann))
#   }
# 
#   for (j in rep_ori){
#     signature_rep_ori_matrix[k,c(1,2,3,4,5,6)] <- metadata[i,c(2,7,29,30,31,32)]
#     signature_rep_ori_matrix[k,7] <- j
# 
# 
#     nn <- names(table(vcf$mut_sign[vcf$rep_str_ann == j]))
#     vv <- as.vector(table(vcf$mut_sign[vcf$rep_str_ann == j]))
# 
#     signature_rep_ori_matrix[k,nn] <- vv
# 
#     k <- k + 1
#   }
# 
...
# 
# 
# saveRDS(signature_rep_ori_matrix, file = "./sig_rep_ori_matrix.rds")
...
if (dir.exists("/hpc/cuppen/")){
  signature_rep_ori_matrix <- readRDS(file = "/hpc/cuppen/projects/P0013_WGS_patterns_Diagn/misc/processed/ali-proj/analysis/signature-exploration/sig-and-rep-ori/sig_rep_ori_matrix.rds")
} else {
  signature_rep_ori_matrix <- readRDS(file = "/home/ali313/Documents/studies/master/umc-project/hpc/cuppen/projects/P0013_WGS_patterns_Diagn/misc/processed/ali-proj/analysis/signature-exploration/sig-and-rep-ori/sig_rep_ori_matrix.rds")
}
...
for (sample_id in metadata$sampleId){
  ...
  for (mut_sig in mut_sigs){
    ...
    signature_rep_ori_matrix[signature_rep_ori_matrix$sampleId == sample_id, mut_sig] <- signature_rep_ori_matrix[signature_rep_ori_matrix$sampleId == sample_id, mut_sig]/sum(signature_rep_ori_matrix[signature_rep_ori_matrix$sampleId == sample_id, mut_sig])
...
my_df_rep_ori <- data.frame(pathway = NA, rep_ori = NA)
my_df_rep_ori[mut_sigs] <- NA

i <- 0

for (rep_ori in unique(signature_rep_ori_matrix$rep_ori)){
  for (pathway in unique(signature_rep_ori_matrix$pathway)[c(1,2,4,5,6)]){
    
    i <- i + 1
    my_df_rep_ori[i,"pathway"] <- pathway
    my_df_rep_ori[i,"rep_ori"] <- rep_ori
    
    for (mut_sig in mut_sigs){
      print(mut_sig)
      my_df_rep_ori[i,mut_sig] <- mean(signature_rep_ori_matrix[signature_rep_ori_matrix$pathway == pathway & signature_rep_ori_matrix$rep_ori == rep_ori, mut_sig], na.rm = T)
...
# making a tibble
sig_rep_ori_tibble <- gather_(my_df_rep_ori,
                              key_col = "mut_sigs",
                              value_col = "value",
                              gather_cols = colnames(my_df_rep_ori[,3:56]))
...
sig_rep_ori_tibble[,1] <- factor(sig_rep_ori_tibble[,1], levels = c("WT","BER","NER","POL", "MMR"))
sig_rep_ori_tibble[,2] <- factor(sig_rep_ori_tibble[,2], levels = c("-", "left", "right"))
sig_rep_ori_tibble[,3] <- factor(sig_rep_ori_tibble[,3], levels = sig_order)
sig_rep_ori_tibble[,4] <- as.numeric(sig_rep_ori_tibble[,4])



# demo data for plotting
sig_rep_ori_tibble1 <- sig_rep_ori_tibble[sig_rep_ori_tibble$mut_sigs %in% c("SBS1","SBS2","SBS10a","SBS15","SBS44"),]
...
for (i in 1:nrow(data_to_be_removed)) {
  sig_rep_ori_tibble$value[sig_rep_ori_tibble$mut_sigs == data_to_be_removed$mut_sigs[i] & sig_rep_ori_tibble$pathway == data_to_be_removed$pathway[i]] <- NA
}


# plotting

t <- ggplot(sig_rep_ori_tibble, aes(x=pathway, y=value, fill=rep_ori)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) + facet_wrap(~ mut_sigs, ncol = 5) +
  scale_fill_brewer("Replication Strand Orientation", palette = "Pastel2") +
...
if (dir.exists("/hpc/cuppen/")){
  ggsave(filename = "/home/ali313/Documents/studies/master/umc-project/hpc/cuppen/projects/P0013_WGS_patterns_Diagn/misc/processed/ali-proj/analysis/signature-exploration/sig-and-rep-ori/pathway-mutsig-repori-corrected.pdf",
         plot = t, width = 50, height = 110, units = "cm")
} else {
  ggsave(filename = "pathway-mutsig-repori-corrected.pdf", plot = t, width = 50, height = 110, units = "cm")
  ggsave(filename = "/home/ali313/Documents/studies/master/umc-project/hpc/cuppen/projects/P0013_WGS_patterns_Diagn/misc/processed/ali-proj/analysis/signature-exploration/sig-and-rep-ori/pathway-mutsig-repori-corrected.pdf", plot = t, width = 50, height = 110, units = "cm")
}
